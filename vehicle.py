...
class Vehicle(object):
    ...
    def check_collision(self, positions_drones, pos_obstacles, pos_no_flight_zones, index):
        """
        ...
        # --- Repulsion no-fly zones 
        for p in pos_no_flight_zones:
            d = (self.location - p).length()
            factor_repulsion = 0.005
            if (self.location[0] > p[0] and self.location[0] < p[0] + NO_FLIGHT_ZONE_WEIGHT 
                and self.location[1] > p[1] and self.location[1] < p[1] + NO_FLIGHT_ZONE_HEIGHT):
                f_repulsion = derivativeBivariate(factor_repulsion,factor_repulsion, p, self.location )/SAMPLE_TIME
                f_repulsion = limit(f_repulsion,self.max_force*1.8)
             #----
                self.velocity *= -1
